...
interface CardDao: BaseDao<Card> {
    ...
    @Query("select * from list as l, card as c, board as b, member as m, memberboardrel as mb" +
            " where m.email = mb.email and mb.boardId = b.boardId and b.boardId = l.boardId and l.listId = c.listIdPar and c.startDate > 0 and c.dueDate > 0 and c.checked = 0 and c.cardStatus = 'active' and b.boardStatus = 'open' and m.email = :email and c.startDate > :date" +
